...
function buildReportContext(data) {
  ...
  const insights = Array.isArray(data?.global_insights)
  ...
  return {
    ...
    progressItems: detections.filter(
      (det) => det.category === 'progress' && det.progress
    ),
    progressInsights: insights.filter(
      (insight) => insight.category === 'progress'
    ),
    otherInsights: insights.filter(
      (insight) => insight.category !== 'progress'
    ),
  };
}
...
function renderPreprocessingSection(context) {
  ...
  return renderSection(
    ...
    renderPreprocessing(context.preprocessing),
    false
  );
}
...
function renderSafetySection(context) {
  ...
  return renderSection(
    'safety',
    '🚨 Safety Issues',
    renderSafetyCards(context.safetyIssues),
    false
  );
}
...
function renderProgressSectionWrapper(context) {
  if (
    context.progressItems.length === 0 &&
    context.progressInsights.length === 0
  )
    return '';
  return renderSection(
    'progress',
    '📊 Progress',
    renderProgressSection(context.progressItems, context.progressInsights),
    true
  );
}
...
function renderDetectionsSection(context) {
  ...
  return renderSection(
    'detections',
    '🔍 All Detections',
    renderDetectionCards(context.detections),
    true
  );
}
...
function renderInsightsSection(context) {
  ...
  return renderSection(
    'insights',
    '💡 Global Insights',
    renderInsights(context.otherInsights),
    true
  );
}
...
function renderAggregatesSection(context) {
  ...
  if (
    aggregates.countsByLabel.length === 0 &&
    aggregates.countsByCategory.length === 0
  ) {
    return '';
  }
  return renderSection(
...
function addDimensionMetrics(meta, metrics) {
  if (meta.sourceWidth && meta.sourceHeight) {
    metrics.push({
      ...
      value: `${meta.sourceWidth}×${meta.sourceHeight}`,
    });
  }
  if (meta.targetWidth && meta.targetHeight) {
    ...
    metrics.push({
      ...
      value: `${meta.targetWidth}×${meta.targetHeight}`,
      note,
    });
  }
}
...
function renderProgressSection(progressItems, progressInsights) {
  ...
  if (progressItems.length > 0) {
    html +=
      '<h4 style="margin:0 0 12px; font-size:14px; color:#9aa0b4;">Regional Progress</h4>';
  ...
  if (progressInsights.length > 0) {
    if (progressItems.length > 0) {
      html +=
        '<h4 style="margin:20px 0 12px; font-size:14px; color:#9aa0b4;">Overall Progress</h4>';
    }
    progressInsights.forEach((insight) => {
      ...
      let percent = 0;
      if (insight.metrics) {
        const pctMetric = insight.metrics.find(
          (m) =>
            m.key.toLowerCase().includes('percent') ||
            m.key.toLowerCase().includes('complete')
        );
  ...
  return (
    html || '<p style="color:#9aa0b4;">No progress information available.</p>'
  );
}
...
function renderInsights(insights) {
  ...
  return (
    cards || '<p style="color:#9aa0b4;">No additional insights available.</p>'
  );
}
...
function renderAggregates(aggregates) {
  ...
  if (aggregates.countsByCategory.length > 0) {
    html +=
      '<h4 style="margin:0 0 12px; font-size:14px; color:#9aa0b4;">By Category</h4>';
    const maxCount = Math.max(
      ...aggregates.countsByCategory.map((c) => c.count)
    );
  ...
  if (aggregates.countsByLabel.length > 0) {
    html +=
      '<h4 style="margin:20px 0 12px; font-size:14px; color:#9aa0b4;">By Label (Top 10)</h4>';
 ...
 */
export function setupReportInteractions(
  reportContainer,
  detections,
  onHover,
  onLeave
