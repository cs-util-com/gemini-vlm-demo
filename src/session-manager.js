...
const CSV_HEADER = 'Image,File Name,Detections,Safety Issues,High,Medium,Low,Status';
...
function processCompletedImage(state, image, imageNumber) {
	const detections = Array.isArray(image.result?.detections) ? image.result.detections : [];
	...
	state.imagesSafety.push(buildImageSafetySummary(image, detections.length, severityCounts));
...
function applySafetyCounters(state, detection, severityCounts) {
	...
	state.safetyBySeverity[severity] = (state.safetyBySeverity[severity] || 0) + 1;
...
function buildImageSafetySummary(image, detectionsCount, severityCounts) {
	...
	const safetyCount = SAFETY_SEVERITIES.reduce((sum, key) => sum + normalized[key], 0);
	return {
...
function finalizeAggregateState(state) {
	...
	const progressSummary = buildProgressSummary(state.progressEntries, progressByImage, state.phaseCounts, state.progressSourceCounts);
...
function buildProgressSummary(entries, progressByImage, phaseCounts, sourceCounts) {
	const totalEntries = entries.length;
	const averagePercent = totalEntries > 0 ? entries.reduce((sum, entry) => sum + entry.percent, 0) / totalEntries : null;
	const topEntry = totalEntries > 0 ? entries.reduce((max, entry) => (entry.percent > max.percent ? entry : max), entries[0]) : null;
	const bottomEntry = totalEntries > 0 ? entries.reduce((min, entry) => (entry.percent < min.percent ? entry : min), entries[0]) : null;
...
function updateProgressFromInsights(state, image, imageNumber) {
	const insights = Array.isArray(image.result?.global_insights) ? image.result.global_insights : [];
	insights
...
function registerProgressEntry(state, { image, imageNumber, percent, label, phase, source }) {
	if (phase) {
	...
	state.progressSourceCounts[source] = (state.progressSourceCounts[source] || 0) + 1;

	const perImage = ensureProgressImageBucket(state.progressByImage, image, imageNumber);
