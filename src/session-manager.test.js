...
describe('session-manager', () => {
  describe('createSession', () => {
    it('should create a session with correct structure', () => {
      const files = [{ name: 'img1.jpg' }, { name: 'img2.jpg' }];
  ...
  describe('calculateSessionAggregates', () => {
    ...
    it('should aggregate progress percentages across detections and insights', () => {
      ...
      expect(summary.byImage[0]).toMatchObject({
        imageNumber: 2,
        averagePercent: 80,
      });
      expect(
        summary.phaseCounts.find((phase) => phase.name === 'Framing')?.count
      ).toBeGreaterThan(0);
      expect(
        summary.phaseCounts.find((phase) => phase.name === 'Finishing')?.count
      ).toBeGreaterThan(0);
    ...
    it('should normalize progress values and track phases without explicit percentages', () => {
      ...
      expect(summary.byImage[0].averagePercent).toBeCloseTo(
      ...
      expect(summary.sourceCounts.insight).toBe(1);
      expect(
        summary.phaseCounts.find((phase) => phase.name === 'Drywall')?.count
      ).toBe(1);
    });
    ...
    it('should fill in defaults when detection metadata is missing', () => {
      ...
      const categoryOther = aggregates.countsByCategory.find(
        (item) => item.category === 'other'
      );
      const labelUnknown = aggregates.countsByLabel.find(
  ...
  describe('exportSessionCSV', () => {
    it('should export session summary as CSV', () => {
      ...
      updateImageStatus(session, 'img_002', 'completed', {
        detections: [{ id: '3', label: 'tool', category: 'object' }],
      });
    ...
    it('should throw if aggregates not calculated', () => {
      ...
      expect(() => exportSessionCSV(session)).toThrow(
        'Session aggregates not calculated'
      );
    });
