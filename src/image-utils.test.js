...
describe('computeResizeDimensions', () => {
  ...
  test('downscales panoramic images but preserves minimum short side', () => {
    ...
    expect(result.height).toBeGreaterThanOrEqual(
      IMAGE_PREPROCESS_DEFAULTS.minShortSide
    );
    expect(result.strategy).toBe('downscale-dual-axis');
  });
  ...
  test('throws when height is invalid', () => {
    ...
    expect(() =>
      computeResizeDimensions(800, Number.POSITIVE_INFINITY)
...
describe('downscaleImageForGemini', () => {
  ...
  function mockCanvas({ toBlobImpl, convertToBlobImpl } = {}) {
    const canvas = {
      ...
      toBlob: jest.fn(
        toBlobImpl ??
          ((cb) => cb(new Blob([new Uint8Array(256)], { type: 'image/webp' })))
      ),
    };
  ...
  test('rejects when provided non-blob input', async () => {
    await expect(downscaleImageForGemini('not-a-blob')).rejects.toThrow(
      'Invalid file'
    );
  });
  ...
  test('falls back to original payload when conversion is not smaller', async () => {
    ...
    const file = new File([new Uint8Array(2048)], 'photo.jpg', {
      type: 'image/jpeg',
    });
    const result = await downscaleImageForGemini(file);
  ...
  test('emits warnings for panoramic scenes requiring many tiles', async () => {
    ...
    const file = new File([new Uint8Array(1024)], 'wide.jpg', {
      type: 'image/jpeg',
    });
    const result = await downscaleImageForGemini(file);
  ...
  test('records warning when canvas conversion fails', async () => {
    ...
    const file = new File([new Uint8Array(3072)], 'error.jpg', {
      type: 'image/jpeg',
    });
    const result = await downscaleImageForGemini(file);
    ...
    expect(result.resized).toBe(false);
    expect(
      result.warnings.some((w) => /Failed to downscale image/i.test(w))
    ).toBe(true);
  });
  ...
  test('uses convertToBlob when available and flags large payload warning', async () => {
    ...
    const file = new File([new Uint8Array(4 * 1024 * 1024)], 'convert.jpg', {
      type: 'image/jpeg',
    });
    const result = await downscaleImageForGemini(file);
  ...
  test('warns when inline payload exceeds 20 MB', async () => {
    ...
    const file = new File([bigBytes], 'large-inline.jpg', {
      type: 'image/jpeg',
    });
    const result = await downscaleImageForGemini(file);
  ...
  test('falls back to HTML image decoding when bitmap creation fails', async () => {
    ...
    const file = new File([new Uint8Array(512)], 'fallback.jpg', {
      type: 'image/jpeg',
    });
    const result = await downscaleImageForGemini(file);
...
describe('estimateTileFootprint', () => {
  ...
  test('throws when width is not a positive finite number', () => {
    ...
    expect(() => estimateTileFootprint(Number.NaN, 100)).toThrow(
      'Invalid width'
    );
  });
  ...
  test('throws when height is not a positive finite number', () => {
    ...
    expect(() => estimateTileFootprint(100, Number.POSITIVE_INFINITY)).toThrow(
      'Invalid height'
    );
  });

  test('throws when tile size is not a positive finite number', () => {
    expect(() => estimateTileFootprint(100, 100, 0)).toThrow(
      'Invalid tileSize'
    );
    expect(() => estimateTileFootprint(100, 100, Number.NaN)).toThrow(
      'Invalid tileSize'
    );
  });
});
