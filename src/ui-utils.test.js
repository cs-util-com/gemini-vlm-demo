...
describe('extractJSONFromResponse', () => {
  ...
  it('throws error when no candidates', () => {
    expect(() => extractJSONFromResponse({})).toThrow(
      'No text JSON found in response.'
    );
  });

  it('throws error when candidates array is empty', () => {
    expect(() => extractJSONFromResponse({ candidates: [] })).toThrow(
      'No text JSON found in response.'
    );
  });
  ...
  it('strips truncated mask data so remaining fields can be parsed', () => {
    const truncated =
      '{"items":[{"label":"skylight","box_2d":[0,482,453,997],"category":"facility_asset","mask":"\\u003cstart_of_mask\\u003eABC';
    const resp = {
      candidates: [
        {
          content: {
  ...
  it('recovers from truncated mask asset object payloads', () => {
    const truncated =
      '{"items":[{"label":"panel","category":"object","mask":{"inline_data":{"mime_type":"image/png","data":"AAA';
    const resp = {
      candidates: [
        {
          content: {
...
describe('calculateDisplayScale', () => {
  ...
  it('throws error for invalid naturalWidth', () => {
    expect(() => calculateDisplayScale(0, 1920)).toThrow(
      'Invalid naturalWidth'
    );
    expect(() => calculateDisplayScale(-100, 1920)).toThrow(
      'Invalid naturalWidth'
    );
    expect(() => calculateDisplayScale(NaN, 1920)).toThrow(
      'Invalid naturalWidth'
    );
    expect(() => calculateDisplayScale(Infinity, 1920)).toThrow(
      'Invalid naturalWidth'
    );
    expect(() => calculateDisplayScale('800', 1920)).toThrow(
      'Invalid naturalWidth'
    );
  });

  it('throws error for invalid viewportWidth', () => {
    expect(() => calculateDisplayScale(800, 0)).toThrow(
      'Invalid viewportWidth'
    );
    expect(() => calculateDisplayScale(800, -1920)).toThrow(
      'Invalid viewportWidth'
    );
    expect(() => calculateDisplayScale(800, NaN)).toThrow(
      'Invalid viewportWidth'
    );
  });

  it('throws error for invalid padding', () => {
    expect(() => calculateDisplayScale(800, 1920, -10)).toThrow(
      'Invalid padding'
    );
    expect(() => calculateDisplayScale(800, 1920, NaN)).toThrow(
      'Invalid padding'
    );
    expect(() => calculateDisplayScale(800, 1920, Infinity)).toThrow(
      'Invalid padding'
    );
  });
});
...
describe('extractBase64FromDataUrl', () => {
  ...
  it('throws error for non-string input', () => {
    expect(() => extractBase64FromDataUrl(123)).toThrow(
      'Invalid dataUrl: must be a string'
    );
    expect(() => extractBase64FromDataUrl(null)).toThrow(
      'Invalid dataUrl: must be a string'
    );
    expect(() => extractBase64FromDataUrl(undefined)).toThrow(
      'Invalid dataUrl: must be a string'
    );
  });

  it('throws error for invalid format without comma', () => {
    expect(() => extractBase64FromDataUrl('invalidformat')).toThrow(
      'Invalid dataUrl format'
    );
  });

  it('throws error for empty string', () => {
    expect(() => extractBase64FromDataUrl('')).toThrow(
      'Invalid dataUrl format'
    );
  });
});
...
describe('prepareDetectionData', () => {
  ...
  it('throws error for invalid parsed data', () => {
    expect(() => prepareDetectionData(null, 1920, 1080)).toThrow(
      'Invalid parsed data'
    );
    expect(() => prepareDetectionData(undefined, 1920, 1080)).toThrow(
      'Invalid parsed data'
    );
    expect(() => prepareDetectionData('string', 1920, 1080)).toThrow(
      'Invalid parsed data'
    );
  });

  it('throws error for invalid naturalWidth', () => {
    expect(() => prepareDetectionData({}, 0, 1080)).toThrow(
      'Invalid naturalWidth'
    );
    expect(() => prepareDetectionData({}, -100, 1080)).toThrow(
      'Invalid naturalWidth'
    );
    expect(() => prepareDetectionData({}, NaN, 1080)).toThrow(
      'Invalid naturalWidth'
    );
  });

  it('throws error for invalid naturalHeight', () => {
    expect(() => prepareDetectionData({}, 1920, 0)).toThrow(
      'Invalid naturalHeight'
    );
    expect(() => prepareDetectionData({}, 1920, -100)).toThrow(
      'Invalid naturalHeight'
    );
    expect(() => prepareDetectionData({}, 1920, Infinity)).toThrow(
      'Invalid naturalHeight'
    );
  });
...
describe('transformResponseFormat', () => {
  ...
  it('includes mask when present', () => {
    ...
    expect(result.detections[0]).toHaveProperty(
      'mask',
  ...
  it('resolves mask references from mask_assets map', () => {
    ...
    expect(result.detections[0].mask.startsWith('data:image/png;base64,')).toBe(
      true
    );
  });
  ...
  it('supports inline_data mask objects', () => {
    ...
    expect(result.detections[0]).toHaveProperty(
      'mask',
      'data:image/webp;base64,TWFuIGlzIGRpc3Rpbmd1aXNoZWQ='
    );
  });
  ...
  it('includes optional metadata fields when present', () => {
    ...
    expect(detection.safety).toEqual({
      severity: 'high',
      actions: ['evacuate'],
    });
    expect(detection.progress).toEqual({
      status: 'blocked',
      percent_complete: 35,
    });
    expect(detection.attributes).toEqual([{ name: 'color', value: 'yellow' }]);
    expect(detection.relationships).toEqual([
      { type: 'near', target: 'det_7' },
    ]);
  });
  ...
  it('returns input unchanged if already in legacy format', () => {
    const input = {
      detections: [
        { id: '1', label: 'test', category: 'object', confidence: 0.9 },
      ],
      global_insights: [],
    };
  ...
  it('gathers mask assets from multiple sources', () => {
    ...
    const input = {
      ...
      assets: {
        maskAssets: {
          mask_from_assets_url: { url: 'https://cdn.example.com/mask.png' },
        },
        mask_assets: {
          mask_from_assets_inline: {
            inlineData: { data: base, mimeType: 'image/avif' },
          },
        },
    ...
    expect(Object.keys(result.maskAssets)).toEqual(
    ...
    const input = {
      items: [
        { labels: ['dataUrl'], box_2d: [1, 1, 2, 2], mask: dataUrl },
        {
          labels: ['inlineCamel'],
          box_2d: [2, 2, 3, 3],
          mask: { inlineData: { data: baseInline, mimeType: 'image/gif' } },
        },
        {
          labels: ['directObject'],
          box_2d: [3, 3, 4, 4],
        ...
        },
        {
          labels: ['assetBase64'],
          box_2d: [5, 5, 6, 6],
          mask: 'mask_asset_base64',
        },
        {
          labels: ['assetPngBase64'],
          box_2d: [6, 6, 7, 7],
          mask: 'mask_asset_png_base64',
        },
        {
          labels: ['assetBytes'],
          box_2d: [7, 7, 8, 8],
          mask: 'mask_asset_bytes',
        },
        { labels: ['unresolvedShort'], box_2d: [8, 8, 9, 9], mask: 'short' },
        {
          labels: ['invalidObject'],
          box_2d: [9, 9, 10, 10],
          mask: { foo: 'bar' },
        },
  ...
  it('handles malformed mask assets gracefully', () => {
    ...
    const input = {
      items: [
        ...
        {
          labels: ['numberAsset'],
          mask: 'mask_number_asset',
          box_2d: [0, 0, 1, 1],
        },
        {
          labels: ['inlineBase64'],
          mask: 'mask_inline_base64',
          box_2d: [0, 0, 1, 1],
        },
        {
          labels: ['inlineBytes'],
          mask: 'mask_inline_bytes',
          box_2d: [0, 0, 1, 1],
        },
        {
          labels: ['dataUrlAsset'],
          mask: 'mask_data_url_asset',
          box_2d: [0, 0, 1, 1],
        },
        {
          labels: ['emptyStringAsset'],
          mask: 'mask_empty_string',
          box_2d: [0, 0, 1, 1],
        },
      ],
      mask_assets: {
        ...
        mask_inline_base64: {
          inline_data: { base64: inlineBase64, mime_type: 'image/heif' },
        },
        mask_inline_bytes: {
          inlineData: { bytes: inlineBytes, mimeType: 'image/tiff' },
        },
