...
describe('calculateAggregates', () => {
  ...
  test('handles detections with missing categories', () => {
    ...
    expect(result.countsByCategory).toContainEqual({
      category: 'object',
      count: 1,
    });
    expect(result.countsByCategory).toContainEqual({
  ...
  test('handles single detection', () => {
    ...
    expect(result.countsByLabel).toEqual([{ label: 'ladder', count: 1 }]);
    expect(result.countsByCategory).toEqual([{ category: 'object', count: 1 }]);
  });
  ...
  test('handles duplicate entries correctly', () => {
    ...
    expect(result.countsByLabel).toEqual([{ label: 'ladder', count: 4 }]);
    expect(result.countsByCategory).toEqual([{ category: 'object', count: 4 }]);
  });
});
