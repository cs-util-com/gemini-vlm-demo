...
function buildSessionSummaryContext(session) {
  ...
  return {
    ...
    nonProgressInsights: globalInsights.filter(
...
function renderSummaryCards(session, context) {
  ...
  if (shouldShowProgressCard(context.progressSummary)) {
    const label =
      context.overallProgressPercent != null
...
function shouldShowProgressCard(progressSummary) {
  return (
    progressSummary.totalEntries > 0 ||
    Number.isFinite(progressSummary.averagePercent)
  );
}
...
function renderImagesSafetyHeatmap(imagesSafety) {
  ...
  const chips = imagesWithSafety
    .map((img) => {
      const icon =
        img.maxSeverity === 'high'
          ? '🔴'
          : img.maxSeverity === 'medium'
            ? '🟡'
            : '🟢';
 ...
 */
export function renderImageSectionHeader(
  imageId,
  fileName,
  imageNumber,
  options = {}
  ...
  const detectionMeta =
    typeof detectionCount === 'number' ? `${detectionCount} detections` : null;
...
function renderSessionAggregateCharts(agg) {
  const hasCategory =
    Array.isArray(agg.countsByCategory) && agg.countsByCategory.length > 0;
  const hasLabel =
    Array.isArray(agg.countsByLabel) && agg.countsByLabel.length > 0;
  ...
  if (hasLabel) {
    html +=
      '<h3 class="session-panel-title" style="margin-top:20px;">Top Labels (Session)</h3>';
...
function buildProgressPanelContext(progressSummary) {
  ...
  const roundedPercent = hasPercent
    ? Math.round(progressSummary.averagePercent)
    : null;
...
function renderProgressHeader(context) {
  const label = context.hasPercent
    ? `${context.roundedPercent}% complete`
    : 'Progress signals detected';
  return `
...
function buildProgressPhaseTags(phases) {
  ...
  return phases
    ...
    .map(
      (phase) =>
        `<span class="session-progress-phase">${escapeHtml(phase.name)} (${phase.count})</span>`
    )
    .join(' ');
}
...
function collectSessionGlobalInsights(session) {
  ...
  session.images.forEach((img, idx) => {
    ...
    const list = Array.isArray(img.result.global_insights)
      ? img.result.global_insights
      : [];
    ...
    .map((insight) => {
      ...
      const metricsHtml =
        Array.isArray(insight.metrics) && insight.metrics.length > 0
          ? `<div class="insight-metrics">${insight.metrics
              .map((metric) => {
