...
export function computeResizeDimensions(width, height, options = {}) {
  ...
  const { targetWidth, targetHeight } = computeScaledDimensions(
    width,
    height,
    scale
  );
  const strategy = selectStrategy(
    metrics.withinShortTarget,
    metrics.withinLongTarget
  );
  return {
    scale,
    width: targetWidth,
...
function finalScaleFor(metrics, cfg, candidates) {
  ...
  const withMinShortSide = enforceMinShortSide(
    base,
    metrics.shortSide,
    cfg.minShortSide
  );
  const noUpscale =
    cfg.allowUpscale === false
      ? Math.min(withMinShortSide, 1)
      : withMinShortSide;
 ...
 */
export function estimateTileFootprint(
  width,
  height,
  tileSize = IMAGE_PREPROCESS_DEFAULTS.tileSize
) {
...
export async function downscaleImageForGemini(file, options = {}) {
  ...
  const { converted, error } = await attemptResizeConversion(
    source,
    resize,
    cfg
  );
  const payload = choosePayloadBlob(
    file,
    converted,
    resize,
    cfg,
...
function choosePayloadBlob(
  ...
  if (
    cfg.preferSmallerBytes &&
    sourceBytes != null &&
    convertedBlob.size >= sourceBytes
  ) {
    return {
      blob: originalFile,
      mimeType: baseMime,
...
function buildPreprocessSummary({
  source,
  resize,
  cfg,
  payload,
  ...
  const footprint = estimateTileFootprint(
    targetWidth,
    targetHeight,
    cfg.tileSize
  );
  const compressionRatio =
    sourceBytes && targetBytes ? targetBytes / sourceBytes : null;
...
function collectPreprocessWarnings({
  footprint,
  targetBytes,
  conversionError,
  resize,
  ...
  if (footprint.totalTiles > 4) {
    warnings.push(
      'Image spans more than four 768px tiles; consider cropping regions of interest for higher fidelity.'
    );
  }
  if (targetBytes != null && targetBytes > 18 * 1024 * 1024) {
    warnings.push(
      'Inline payload is approaching the 20 MB limit. Consider additional compression or the Files API.'
    );
  }
  ...
  if (!payload.resized && payload.discardReason === 'larger-than-source') {
    warnings.push(
      'Downscaled image exceeded original file size; kept original bytes instead.'
    );
  }
  return warnings;
}
