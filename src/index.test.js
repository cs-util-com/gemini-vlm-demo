...
describe('geometry helpers', () => {
  ...
  describe('Gemini bbox array format [ymin, xmin, ymax, xmax]', () => {
    ...
    test('toCanvasBox returns null for invalid array', () => {
      expect(
        toCanvasBox([1, 2, 3], 'normalized_0_1000', 1, 1, naturalW, naturalH)
      ).toBeNull();
      expect(
        toCanvasBox(
          [NaN, 100, 200, 300],
  ...
  test('toCanvasBox returns null when bbox contains non-finite values', () => {
    ...
    expect(
      toCanvasBox(bbox, 'pixel', 1, 1, naturalW, naturalH, 'top-left', 200, 100)
    ).toBeNull();
  });
  ...
  test('toCanvasPolygon returns null when poly is not an array', () => {
    expect(
      toCanvasPolygon(null, 'pixel', 1, 1, naturalW, naturalH, 'top-left')
    ).toBeNull();
  });
  ...
  test('ensureCoordSystem defaults to normalized_0_1000 (Gemini standard)', () => {
    const parsed = { image: {} };
    expect(ensureCoordSystem(parsed, 'normalized_0_1000')).toBe(
      'normalized_0_1000'
    );
  });
  ...
  describe('Integration: Full Gemini response flow', () => {
    test('Complete flow from Gemini response to canvas coordinates', () => {
      ...
      const coordSystem = ensureCoordSystem(
        geminiResponse,
        'normalized_0_1000'
      );
